...
async function extractWeaponData() {
  ...
  const data = await Promise.all(categories.map(async c => {
    ...
    const text = await loadFile(`${character}-${category}.tsv`)
    const enText = await loadFile(`${character}-${category}-en.tsv`)
...
export default async function extractCalcdata() {
  throw new Error('weapons-1.json was manually altered and should not be further altered')
  const [
    weapons,
  ] = await Promise.all([
    extractWeaponData(),
