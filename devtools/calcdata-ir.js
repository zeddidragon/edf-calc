import { setGame, loadFile } from './load-sgott.js'
setGame('ir')
...
async function extractGunStats(category) {
  const data = JSON.parse(await loadFile(`${category}.json`))
  const textEn = JSON.parse(await loadFile('en-Text_Name.json'))
  const textJa = JSON.parse(await loadFile('ja-Text_Name.json'))
  const dmgs = JSON.parse(await loadFile('DamageParam.json'))
